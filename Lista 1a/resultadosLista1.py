...
def preco_com_desconto(preco_original, percentual_desconto):
    ...
    """
    descontoValor = preco_original * (percentual_desconto/100)
    novoValor = preco_original - descontoValor
    return(round(novoValor,2))
...
def dias_perdidos_por_fumar(cigarros_fumados_por_dia, anos_fumando):
    ...
    """
    totalCigarros = (cigarros_fumados_por_dia * 365) * anos_fumando
    diasVidaPerdidos = int((totalCigarros * 10) / 1440)
    return(diasVidaPerdidos)
...
def dois_elevado_a_dez_mil():
    ...
    """
    resultadoDoisElevadoDezMil = len(str(2**10000))
    return(resultadoDoisElevadoDezMil)
...
def media_final_aprovado_reprovado(p1, p2, ep1, ep2):
    ...
    """
    media = ((p1 * 7) + (p2 *7) + (ep1 * 3) + (ep2 * 3)) / 20
    if(media>=7):
        return(True)
    else:
        return(False)
...
def tinta(metros_pintar):
    ...
    latasTinta = math.ceil(metros_pintar/54)
    return(latasTinta)
