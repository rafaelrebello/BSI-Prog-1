...
def intercalamento_listas(lista1, lista2):
    ...
    for i in range(len(lista1)):
        listaResp.append(lista1[i])
        listaResp.append(lista2[i])
    return listaResp
...
def dar_troco(valor_a_pagar, valor_em_dinheiro):
    ...
    """
    listaDeTroco = [[50, 0], [20, 0], [10, 0], [5, 0], [2, 0], [1, 0]]
    listaResp = []
    if valor_a_pagar == valor_em_dinheiro:
        return []
    else:
        valorTroco = valor_em_dinheiro - valor_a_pagar
        for i in range(6):
            listaDeTroco[i][1] = valorTroco//listaDeTroco[i][0]
            if listaDeTroco[i][1] != 0:
                listaResp.append((listaDeTroco[i][0],listaDeTroco[i][1]))
            valorTroco = valorTroco%listaDeTroco[i][0]
...
def media_anual(temperaturas):
    ...
    """
    mesesAcima = []
    mediaTempAno = sum(temperaturas)/len(temperaturas)
    print(len(temperaturas))
    for i in range(len(temperaturas)):
        if temperaturas[i] > mediaTempAno:
            mesesAcima.append(i)
...
def testa_primo(valor):
    ...
    return valor > 1 and verificaPrimo
...
def lista_de_primos(inicio, fim):
    ...
    for i in range(inicio, fim+1):
        ...
        for j in range(2,(i)//2 +1):
            if i%j == 0:
                verificaPrimo = False
        if verificaPrimo and i>1: listaResultado.append(i)
...
def fibonacci(n):
    ...
    listaRetorno = []
    for i in range(n):
        listaRetorno.append(1) if i<2 else listaRetorno.append(listaRetorno[(i-1)] + listaRetorno[(i-2)])
...
def altera_salarios(salarios):
    ...
    for i in range(len(salarios)):
        if salarios[i] <= salarioMin:
            salarios[i] = round(salarios[i]*1.20,2)
        elif salarios[i] <= salarioMin*2:
            salarios[i] = round(salarios[i]*1.15,2)
        elif salarios[i] <= salarioMin*5:
            salarios[i] = round(salarios[i]*1.10,2)
        else:
            salarios[i] = round(salarios[i]*1.05,2)
    return salarios
