...
def palindromo(texto):
    ...
    texto = texto.lower()
    mytable = texto.maketrans("", "", "!?@, .")
    texto = texto.translate(mytable)
    inverso = texto[::-1]
    return inverso == texto
...
def troca_caixa(texto):
    ...
    """
    texto = texto.lower()
    mytable = texto.maketrans("aeiou", "AEIOU")
    print(mytable)
...
def media_temperaturas(temperaturas):
    ...
    """
    soma_de_valores = sum(temperaturas)
    quant_valores = len(temperaturas)
    return (soma_de_valores/quant_valores)
